# WEBPACK5 FULL PROJECT SETUP - HTML + SASS + Vanilla JS + Bootstrap 5 Grid System

Bu proje yapısında HTML, SASS(SCSS), Vanilla JS ve Bootstrap 5 Grid System ile front-end uygulama geliştirebilirsiniz. Bootstrap için sadece grid system entegre edilmiştir. Izgara sistemi üzerinde değişiklik yapmak için `abstracts/variables.scss` dosyasını inceleyebilirsiniz.  Ayrıca sınıfların nasıl oluşturulduğunu `abstracts/mixins.scss` ve `abstracts/functions.scss` dosyalarında görebilirsiniz. Burada bulunan yapıların kullanıldığı yer ise `layout/grid.scss` dosyasıdır. Bu dosyadaki değişkenlerde `abstracts/variables.scss` dosyasından gelmektedir.
...
3. SASS (7-1 Pattern)
4. Normalize.css
5. Bootstrap 5 Grid System
6. PostCSS (Autoprefixer)
7. HMR (Hot Module Replacement)
8. Source Maps
9. Browserslist
10. Images
11. DIST Folder->Automated Cleaning
